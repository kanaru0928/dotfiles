...
  <do>The do tag represents instructions, policies, and explanations that must be strictly observed. The agent must execute and adhere to the contents of the do tag according to the priority definitions.</do>
  <do>The avoid tag represents prohibited items. The agent must not perform the actions or use the expressions described in the contents of the avoid tag. The avoid tag may have an examples tag as a child element. In that case, examples provides specific examples of the prohibition.</do>
...
  <do>Reader-friendly code — the next engineer can understand it immediately.</do>
  <do>Quality first — pursue the highest standards without compromise.</do>
  <do>Simplicity is justice (KISS) — the simplest implementation with clear intent.</do>
  <do>Boy Scout Rule — leave touched modules "cleaner than when you found them".</do>
  <avoid>Adding unnecessary external dependencies (keep dependencies to an absolute minimum).</avoid>
  <do>Emphasis on consistency — consistency with existing code over code beauty.</do>
  <do>Pattern reference — follow patterns from other files in the project.</do>
</rules>

<purpose>Ironclad Rules for Interpreting Requests</purpose>

<rules priority="GLOBAL MUST" name="Handling Ambiguous Requests">
  <do>No guessing: always ask questions about unclear points.</do>
  <do>Prioritize confirmation: verify by asking "Is it correct to understand that...?"</do>
  <do>Minimal execution: execute only what is explicitly requested.</do>
</rules>

<rules priority="GLOBAL MUST" name="Prohibited Actions Checklist">
  <avoid>Adding unrequested features.</avoid>
  <avoid>Arbitrary refactoring.</avoid>
  <avoid>Implementation based on guessing.</avoid>
  <avoid>Broad interpretation out of helpfulness.</avoid>
